...
having :: forall s. Aggr s Boolean -> Query s Unit
...
notNull :: forall s a. Col s (Maybe a) -> Query s (Col s a)
...
notNull_ :: forall s a. Aggr s (Maybe a) -> Query s (Aggr s a)
...
crossJoin :: forall s r res. FromTable s r res => Table r -> Query s { | res }
crossJoin table = do
  { res, sql } <- fromTable table
...
groupBy :: forall s a. Col s a -> Query s (Aggr s a)
...
orderBy :: forall s a. Order -> Col s a -> Query s Unit
orderBy order (Col e) =
...
offset :: forall s. Int -> Query s Unit
offset i = modify_ \st ->
  case st.limit of
...
instance wrapWithMaybeLeaveMaybe :: Mapping WrapWithMaybe (Col s (Maybe a)) (Col s (Maybe a))
  where
  mapping _ = identity
else instance wrapWithMaybeInstance :: Mapping WrapWithMaybe (Col s a) (Col s (Maybe a))
  where
