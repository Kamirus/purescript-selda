...
class Coerce col where
  ...
  unsafeFromCol :: forall s a. Col s a -> col s a

instance coerceCol :: Coerce Col where
  unsafeFromCol x = x

instance coerceAggr :: Coerce Aggr where
  unsafeFromCol = Aggr

data WrapWithAggr = WrapWithAggr

instance wrapWithAggrInstance :: Mapping WrapWithAggr (Col s a) (Aggr s a)
  where
  mapping _ = Aggr
...
else instance unAggrInstance :: MappingWithIndex UnAggr (Proxy sym) (Aggr s a) (Col s a)
  where
