...
in_
  :: forall s a
   . Col s a
  -> (forall z. FullQuery z { x :: Col z a })
  -> Col s Boolean
...
inArray :: forall s a. Col s a -> Array (Col s a) -> Col s Boolean
inArray (Col e) cols = Col $ EInArray $ mkExists $ InArray e exprs identity
  where
  exprs = map unwrap cols

isNull :: forall s a. Col s (Maybe a) -> Col s Boolean
isNull (Col e) = Col $ EUnOp $ mkExists $ UnExp (IsNull identity) e

isNull_ :: forall s a. Aggr s (Maybe a) -> Aggr s Boolean
